<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%-- 
    Document   : BuyCarport
    Created on : 15-05-2019, 13:12:06
    Author     : Martin & Niels, som så godt ud imens ;) <3
--%>

<!DOCTYPE html>
<h3>Indtast din information</h3>
<div class="Input">
    <label for="Adresse"> Navn </label><br>
    <input id="1" type="text" name="Navn" class="carport" size="65" ><br>
    <label for="length"> Email </label><br>
    <input id="2" type="tel" name="length" class="carport" size="65"><br>
    <label for="width"> Telefonnummer </label><br>
    <input id="3" type="tel" name="width" class="carport" size="65"><br>
    <label for="heigth"> Kodeord </label><br>
    <input id="4" type="password" name="heigth" class="carport" size="65" title="Indtast password her."><br>
    <label for="heigth"> Gentag kodeord </label><br>
    <input id="5" type="password" name="heigth" class="carport" size="65" title="Gentag password her"><br>
    
</div>
<button class="buybutton btn btn-warning" onclick="register()">Registrer som kunde</button>